...
public:
    struct KeyPair {
        std::string publicKey = "64507:65537";
    ...
    static std::string encrypt(const std::string& message, const std::string& pubKey = "64507:65537") {
        auto parts = split(pubKey, ':');
        if (parts.size() != 2) throw std::invalid_argument("Invalid public key format");
        uint64_t n = std::stoull(parts[0]);
        uint64_t e = std::stoull(parts[1]);

        if (n <= 255) throw std::runtime_error("Modulus n must be greater than 255");

        std::string result;
        for (char ch : message) {
            uint64_t byte = static_cast<uint8_t>(ch);
            if (byte >= n) throw std::runtime_error("Message byte too large for modulus");
    ...
    static std::string decrypt(const std::string& cipher, const std::string& privKey) {
        ...
        if (parts.size() != 2) throw std::invalid_argument("Invalid private key format");
        uint64_t n = std::stoull(parts[0]);
        ...
        for (const auto& numStr : numbers) {
            ...
            if (num >= n) throw std::runtime_error("Cipher value too large");
            uint64_t decrypted = modExp(num, d, n);
            if (decrypted > 255) throw std::runtime_error("Decrypted value exceeds byte");
...
private:
    ...
    static uint64_t randomPrime(int bits) {
        if (bits < 2 || bits > 63) throw std::invalid_argument("Prime size must be between 2 and 63 bits");
